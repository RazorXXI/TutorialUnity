...
Aunque es bastante simple el ejemplo, no por ello deja de ser muy aclarador... Basicamente lo que hace es, que si el objeto que porta el script, colisiona contra un objeto, que es pasado por el parámetro de la función como `collision` (tranquilo, que esto no debes tocarlo, Unity lo gestina para que sepa con que se ha chocado), comprueba si el objeto `collision` pertenece al layer 6 (En nuestro ejemplo el layer que tendran los muros y paredes...), y si es asi, pues entonces mandará un mensaje por consola indicando que ha chocado contra un muro. 

Este mensaje, podria haber sido sustituido por una acción `Destroy` que destrozara la pared al chocar con esta, o que eliminara al player si chocaba contra ella, o que le restase vida, etc... no voy a seguir poniendo mas ejemplos, te dejo que te partas tu un ratito la cabecita, pensando en que cosas podrias hacer con las colisiones.
